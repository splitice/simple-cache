...
static const char * test_2x_rb_at_end_rollover_capacity() {
	struct read_buffer rb;
	rbuf_init(&rb);
	RBUF_WRITEMOVE(rb, BUFFER_SIZE+BUFFER_SIZE);
	RBUF_READMOVE(rb, BUFFER_SIZE+1000);
...
static const char * test_2x_rb_initial_empty_at_end() {
	...
	rbuf_init(&rb);
	RBUF_WRITEMOVE(rb, BUFFER_SIZE + BUFFER_SIZE);
	RBUF_READMOVE(rb, BUFFER_SIZE + BUFFER_SIZE);
...
static const char * test_high_rb() {
	struct read_buffer rb;
	rbuf_init(&rb);
	RBUF_WRITEMOVE(rb, 1000);
	RBUF_READMOVE(rb, 65000);

	//Calculations
	mu_assert("test_rb_initial_empty_at_end rbuf_read_to_end check", rbuf_read_to_end(&rb) == 536);
	mu_assert("test_rb_initial_empty_at_end rbuf_read_remaining check", rbuf_read_remaining(&rb) == 1536);
	mu_assert("test_rb_initial_empty_at_end rbuf_write_to_end check", rbuf_write_to_end(&rb) == 2560);
	mu_assert("test_rb_initial_empty_at_end rbuf_write_remaining check", rbuf_write_remaining(&rb) == 2560);
